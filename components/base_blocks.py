...
def CreateMISOEnv(ue_num,bs_num,unit_num,antenna_num,r_min,mec_p_max,transmit_p_max,bw,with_CoMP=True,open_matlab=False,mec_rule="default"):
    # 用来控制什么时候学习
   return IrsCompMISOEnv(bs_num=bs_num, ue_num=ue_num, transmit_p_max=transmit_p_max,mec_p_max=mec_p_max,
                           irs_units_num=unit_num,
                           antenna_num=antenna_num, fov_patch_num=ue_num, reflect_max=2 * np.pi,
                           r_min=r_min,
                           BW=bw,open_matlab=open_matlab,train=False,with_CoMP=with_CoMP,mec_rule=mec_rule)
...
trans_complex_to_real_layer = nn.Sequential(
        nn.Linear(2,1),
        nn.ReLU(True)
    )
    

def trans_complex_to_real(fft_rs):
    mod_matrix = torch.abs(fft_rs)
    #angle_matrix = 1/PI*torch.arccos(fft_rs.real / mod_matrix)
    #H_t = angle_matrix + mod_matrix
    #H_t = fft_rs.real+fft_rs.imag
  
    #H_t = torch.cat((fft_rs.real.cuda(), fft_rs.imag.cuda()),1).cpu()
    #H_t = trans_complex_to_real_layer(H_t)
    H_t = mod_matrix
    return H_t
...
class GANLoss(nn.Module):
    ...
    def __call__(self, prediction, target_is_real):
        ...
        if self.gan_mode in ['lsgan', 'vanilla']:
            ...
            loss = self.loss(prediction, target_tensor)+1e-5
...
def LMMSE_channel_est(pilot_tx, pilot_rx, noise_pwr):
    # pilot_tx: NxPx1xMx2
    # pilot_rx: NxPxS'xMx2
    return complex_multiplication(torch.mean(pilot_rx, 2, True), complex_conjugate(pilot_tx)) / (
                1 + (noise_pwr.unsqueeze(-1) / pilot_rx.shape[2]))
